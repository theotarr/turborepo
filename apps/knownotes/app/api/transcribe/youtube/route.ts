...
export async function POST(req: Request) {
  ...
  const lecture = await db.lecture.create({
    data: {
      // @ts-ignore -- Not sure why this is throwing an error. But it works because there is no dynamic data.
      type: "YOUTUBE",
      title,
      transcript: (transcript as any) ?? undefined,
      userId: session.user.id,
      courseId: courseId ? courseId : undefined, // TODO: Look into why not providing the `courseId` will cause the course to be set to null and throw an error.
      youtubeVideoId: id,
    },
  });
  if (!lecture)
